...
### Fixed
- Let explicit dev stop and replacement recover definitively dead orphaned session records and exact-owned stale routes without signaling persisted PIDs; persist pre-spawn intent, distinguish uncertain process observations, preserve ambiguous legacy records by default, and add a targeted `dev stop --forget-ambiguous-orphans` repair that never signals stored PIDs or overrides a live or uncertain registered identity.
