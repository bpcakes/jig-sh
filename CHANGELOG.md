...
- Keep lease and attempt cache access beneath no-follow repository directory capabilities after workspace-write workers run; require exact same-repository PR head metadata before origin branch mutation; namespace manual occurrence identities at their owning store; and keep acknowledgement/attempt state receipts cancelable without running Git inspection inside the schedule commit boundary.
