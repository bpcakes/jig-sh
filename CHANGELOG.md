...
### Fixed
- Normalize PR head identity from the owner/name fields emitted by GitHub CLI, accept valid failed-check JSON from legacy nonzero `gh pr checks` exits, bind review-reply idempotency to trusted-feedback generation and reply intent, retain pre-existing repair worktrees on unexecuted retries, and serialize PR branch mutation through repository-common lease authority across linked worktrees.
