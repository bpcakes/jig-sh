...
### Fixed
- Normalize PR head identity from the owner/name fields emitted by GitHub CLI, accept valid failed-check JSON from legacy nonzero `gh pr checks` exits, bind review replies and resolution to complete comment generations, preserve non-UTF-8 retained-worktree authority, decode exhausted attempts in Jig UI, report force-with-lease pushes accurately, retain pre-existing repair worktrees on unexecuted retries, and serialize PR branch mutation through repository-common lease authority across linked worktrees.
