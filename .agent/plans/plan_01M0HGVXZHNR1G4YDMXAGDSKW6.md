...
- Observation: process-isolating `vault_tui` preserved the failing terminal frame and showed the browser test stuck in the verified-activity modal after both a bare ESC and an Enter close attempt.
  Evidence: the test synchronized on the audit payload text `field_batch_apply`, which can render before the activity worker has transitioned to the input-ready modal. The close input was therefore sent while the TUI intentionally ignored input; the modal's `Enter/Esc close` footer is the actual readiness marker.
...
- Decision: Wait for the verified-activity modal's input-ready footer, assert the expected audit payload is present, and then close it with Enter.
  Rationale: Payload visibility does not imply that the asynchronous worker has yielded to the interactive modal. Synchronizing on the footer tests the observable state transition and prevents input from racing the worker completion; Enter remains an unambiguous supported close event.
  Date/Author: 2026-08-21 / Codex
