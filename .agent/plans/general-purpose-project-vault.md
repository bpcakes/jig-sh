...
Milestone 3 uses `aho-corasick` 1.1.4 as a direct `jig-vault` dependency for deterministic byte-only leftmost-longest streaming matches. The crate is dual-licensed MIT/Unlicense and declares Rust 1.60 support, so it remains below the workspace's Rust 1.85 minimum. The existing `Redactor` remains appropriate for the broker's capped whole buffers, but its sequential, lossy-text output API cannot preserve arbitrary streamed bytes or matches split across reads; only its bounded raw/encoded pattern generation is shared. The immutable matcher automaton may retain internal copies of patterns that cannot be explicitly zeroized, so exec redaction remains a bounded backup control rather than a secret-isolation boundary.

The first Milestone 4 resolver captured `op` stdout and stderr on reader threads and joined them after the direct child exited. An `op` descendant retaining either pipe could therefore hang the import forever. The final resolver places each Unix `op` invocation in a fresh process group, pumps both pipes nonblockingly with exact caps and zeroizing buffers, performs only a 250 ms final drain, and terminates the owned group after every direct-child completion—even when a background descendant closed all standard streams. Regressions cover both held-pipe and detached descendants. The importer currently fails closed before spawning `op` on non-Unix platforms because its private atomic destination-file guarantees are Unix-only.

Milestone 4 cannot atomically commit a vault under its home and a generated dotenv file in the repository as one filesystem transaction. It therefore prepares and syncs the private destination sibling first, commits all fields under one vault lock, then installs the destination atomically. A race at the final no-clobber install reports that the vault commit succeeded and emits a precomputed exact `--replace --overwrite --home` recovery command. Non-UTF-8 recovery paths are rejected before passphrase capture, external resolution, or mutation rather than rendered lossily.

The first Milestone 5 rekey reused the old envelope's KDF parameters. The final implementation explicitly adopts the current KDF policy while preserving vault identity, timestamps, data-encryption key, state, and audit key, and rotates the salt plus both AEAD nonces. A regression constructs a valid weaker v2 envelope and proves rotation upgrades it.

The final Milestone 5 adversarial pass also found that a stale `JIG_VAULT_NEW_PASSPHRASE` could reach the external `op` importer and that the reusable private-file installer did not bind installation to the temporary inode it created. Successful capture and every vault subprocess now strip both reserved passphrase variables. Private output preparation rejects unsafe shared-writable parents and records/revalidates the temporary's device, inode, type, owner, and mode before installation and cleanup.

Encrypted restore is supported locally only on Linux, where Jig can use `renameat2(RENAME_NOREPLACE)` for an absent-target directory install. Restore reads a bounded regular non-symlink archive, requires a private owned parent, validates and decrypts before staging, writes an identity-tracked `0700` sibling with exact `0600` files, verifies the staged audit chain, appends `backup_restore`, and installs without overwrite. Other platforms fail closed pending equivalent DACL/reparse/no-replace implementations; their compilation remains CI coverage.

Default-parallel core aggregates made pre-existing sub-second broker teardown assertions flaky while many Argon2 tests ran concurrently. Both assertions passed in isolation and the authoritative serial aggregate passed all 193 tests. The repository LOC gate also required the new lifecycle code to be split into focused codec, payload, restore, existing-store, lifecycle, conversion, and test modules; no new `mod.rs` was introduced and the staged LOC gate is green.
...
2026-08-10: OnePassword import output is initially Unix-only because the reusable private-file installer has proven owner-only permissions, symlink refusal, fsync, and atomic no-clobber/overwrite semantics there. Unsupported platforms fail during destination preflight, before passphrase capture or `op` execution. Unix importer subprocesses are isolated in an owned process group that is always terminated after bounded capture, so neither pipe-holding nor fully detached descendants survive a completed `op read`.
