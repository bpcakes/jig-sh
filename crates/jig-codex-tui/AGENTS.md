...
- `src/model/fleet.rs`: Codex-only collective quota forecast outcomes, coverage, and uncertainty labels; `src/model/fleet/cohort.rs` decides eligibility and assembles engine input, and `src/model/fleet/simulation.rs` is the pure reset-aware engine.
...
- Collective quota outcomes or their stated assumptions: `src/model/fleet.rs`; cohort eligibility and sample classification: `src/model/fleet/cohort.rs`.
...
- Engine initialization must obey the invariants the simulation loop obeys. Carrying samples to the forecast origin advances an account's windows together under the same all-windows usability constraint, and a pace still inside its warmup spends no observed quota there even though it still counts toward pooled demand.
