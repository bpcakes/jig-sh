...
#[test]
fn a_warming_pace_cannot_extrapolate_an_account_into_exhaustion() {
    // The older sample reports 10% left with only 60s of its window elapsed, so its 1.5
    // points/second pace is not evidence. Carrying that pace across the 60s alignment
    // interval would erase the whole observed remainder and make the pool look blocked, which
    // is exactly the estimate warmup withholds.
    let rows = [
        row_at(
            "warming",
            ORIGIN - 60,
            account(
                "a@example.com",
                "pro",
                vec![window(90.0, FIVE_HOUR, 17_880)],
            ),
        ),
        row_at(
            "exhausted",
            ...
            account(
                "b@example.com",
                "pro",
                vec![window(100.0, FIVE_HOUR, 9_000)],
            ),
        ),
    ];
...
#[test]
fn alignment_does_not_spend_quota_an_account_could_not_have_used() {
    // The older account has 1% of its short window left but an exhausted weekly window, so it
    // could not have served work during the 30s alignment interval. Consuming that 1% anyway
    // would strand the account after its weekly allowance returns and delay modeled recovery
    // from the weekly reset to the far later short-window reset.
    let rows = [
        row_at(
            "blocked",
            ORIGIN - 30,
            account(
                "a@example.com",
                "pro",
                vec![window(99.0, FIVE_HOUR, 16_170), window(100.0, WEEKLY, 300)],
            ),
        ),
        row_at(
            "spent",
            ...
            account(
                ...
                "pro",
                vec![
                    window(100.0, FIVE_HOUR, 9_000),
                    window(100.0, WEEKLY, 302_400),
                ],
            ),
        ),
    ];
    ...
    assert_near(
        recovers_at.expect("the weekly reset restores the retained short-window allowance"),
        ORIGIN + 300,
    );
}

#[test]
fn an_account_blocked_through_alignment_still_contributes_its_demand() {
    // Alignment must stop spending a blocked account's allowance without also dropping its
    // estimated share of fleet demand.
    let blocked = row_at(
        "blocked",
        ORIGIN - 30,
        account(
    ...
    let spare = row_at(
        ...
        account(
            "b@example.com",
            "pro",
            vec![window(0.0, FIVE_HOUR, 9_000), window(0.0, WEEKLY, 302_400)],
        ),
    );

    // The spare account alone reports no burn at all, so any gap below comes from the blocked
    // account's retained pace.
    assert_eq!(
        forecast(std::slice::from_ref(&spare)).outcome,
        FleetOutcome::NoGap {
            burn_observed: false
    ...
    let FleetOutcome::GapRisk { gap_at, .. } = forecast(&[blocked, spare]).outcome else {
        panic!("retained demand should exhaust the spare account inside the horizon");
    };
    assert!(gap_at < ORIGIN + FIVE_HOUR * 60, "gap at {gap_at}");
}
