...
#[test]
fn a_stale_sample_states_its_alignment_limit_instead_of_a_horizon_limit() {
    // A deliberately nonstandard one-minute window renews hundreds of times between the
    // older sample and the common origin. Reaching the alignment guard is a different failure
    // from exhausting the subsequent forecast-horizon work budget.
    let observed_at = ORIGIN - 600 * 60;
    let rows = [
        row_at(
            "stale",
            observed_at,
        ...
        // The current sample is deliberately inside warmup. Collecting pace evidence must
        // not hide a failure to align the older sample.
        row(
            "current",
            account("b@example.com", "pro", vec![window(1.0, 1, 60)]),
        ),
    ];
