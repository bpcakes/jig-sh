...
fn draw_content(frame: &mut Frame, area: Rect, app: &App) {
    let missing_active_domain = match app.tab {
        Tab::Status | Tab::Packages | Tab::Blockers => app.status.data.is_none(),
        Tab::Work | Tab::Timeline | Tab::Health => app.recorder.data.is_none(),
    };
    if missing_active_domain {
        let message = app
            .domain(app.tab)
            .error
...
fn issue_lines(app: &App) -> Vec<Line<'static>> {
    ...
    if let Some(dashboard) = &app.status.data {
...
fn draw_package_table(frame: &mut Frame, area: Rect, app: &App, packages: &[&PackageView]) {
    ...
    let rows = packages.iter().map(|package| {
        let mut cells = vec![
            Cell::from(package.display_id.clone()),
...
fn draw_package_preview(frame: &mut Frame, area: Rect, package: Option<&PackageView>) {
    ...
    for blocker in &package.blockers {
        lines.push(Line::from(vec![
            Span::styled(
                format!("BLOCKER {}: ", blocker.display_code),
                Style::default().fg(BAD).bold(),
            ),
            Span::raw(format!(
                "{}{}{}",
        ...
        let items = blockers
            ...
            .map(|item| {
                ListItem::new(Line::from(vec![
                    Span::styled(
                        format!("{} ", item.display_package_id),
                        Style::default().fg(ACCENT),
                    ),
                    Span::styled(
                        format!("[{}] ", item.blocker.display_code),
                        Style::default().fg(BAD),
                    ),
...
fn draw_blocker_detail(frame: &mut Frame, area: Rect, item: Option<&BlockerItemView>) {
    ...
    if let Some(related) = &item.blocker.display_related_work_package {
...
fn draw_footer(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let first = Line::from(vec![
        ...
        Span::raw(format!(
            "  r refresh  R refresh all  Tab views{provider_help}{selection_help}"
        )),
    ]);
    let domain = app.domain(app.tab);
    let second = if let Some(error) = domain.error {
        Line::from(vec![
            Span::styled("Last refresh error: ", Style::default().fg(BAD)),
            Span::raw(error.to_owned()),
        ])
    } else if domain.refresh_queued {
...
fn repository_label(dashboard: &super::model::Dashboard) -> String {
    let repo = &dashboard.repository;
    let branch = repo.branch.as_deref().unwrap_or(if repo.detached {
        "detached"
    } else {
