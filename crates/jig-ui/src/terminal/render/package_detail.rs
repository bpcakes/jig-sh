...
use super::super::model::{
    App, DETAIL_SECTION_ITEM_LIMIT, EXTENSION_ROW_LIMIT, FacetView, PackageView, SourceView, Tab,
    sanitize_text,
};
...
pub(super) fn draw(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let sizing_paragraph = Paragraph::new(lines.clone()).wrap(Wrap { trim: false });
    ...
    let title = format!(
        ...
        bounded_text(&package.display_id)
    );
    frame.render_widget(
        Paragraph::new(lines)
            .block(panel(&title))
...
fn detail_lines(app: &App, package: &PackageView) -> Vec<Line<'static>> {
    ...
    let mut lines = vec![
        Line::from(vec![
            Span::styled(
                format!("{}  ", bounded_text(&package.display_id)),
    ...
    if package.blockers.is_empty() {
        ...
        for blocker in package.blockers.iter().take(DETAIL_SECTION_ITEM_LIMIT) {
            ...
            lines.push(Line::from(vec![
                Span::styled(
                    format!("{}: ", bounded_text(&blocker.display_code)),
