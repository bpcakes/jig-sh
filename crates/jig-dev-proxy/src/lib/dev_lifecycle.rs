...
fn dead_unconfirmed_cleanup_retires_the_orphan_and_only_its_owned_routes() {
    ...
    let persisted = store.snapshot_dev_state().unwrap();
    assert_unconfirmed_orphan(&persisted);
    configure_dead_orphan_routes(&store);
    ...
    .unwrap();
    assert_recoverable_status(&status);
    ...
    .unwrap();
    assert_dead_orphan_recovery(&stopped);
    assert_only_unowned_routes_remain(&store);
}
