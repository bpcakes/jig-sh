...
- A running proxy's effective LAN bind, HTTPS listener, and HTTPS HTTP/2 setting are reported by its versioned capability response only to loopback clients with the private health token. Reuse must match that response to the authenticated health PID and current runtime token/PID, reject unknown or incompatible settings before app spawn or route publication, and never restart a shared proxy automatically. Preserve the older PID health response for existing clients.
- Inconclusive health with remaining runtime artifacts blocks background daemon startup without removing those artifacts. Prepare app-specific certificates only after compatible proxy readiness, and recheck that readiness under the certificate lock before any write; a health timeout is not proof of absence.
...
- Version 1 sessions may lack preflight cleanup evidence even when app spawn tracking is present. Treat a missing preflight field as unknown for strict orphan retirement; only the existing explicit stop ambiguity repair may waive it. Version 2 requires explicit cleanup, preflight, and app spawn/process evidence. Preserve a populated version 1 document during inspection and explicit cleanup. Enable empty-store version 2 promotion at claim time only after repository-independent exact-session discovery and repair are available; never downgrade version 2 or upgrade beneath active old supervisors.
- Only the live foreground supervisor may terminate its registered app trees. Management reaches it through the authenticated loopback control channel; persisted PID and start-token observations are never external signaling authority. Persist spawn-pending state before creating each app process and clear it only when the exact process identity is durable. When the supervisor is gone, retain `cleanup_required` records while spawn state is pending or unknown, or while any exact registered identity remains live or cannot be classified safely. Once spawn state is known and every registered identity is absent, ordinary same-repository `dev` launch, explicit `dev stop`, or `dev --replace` may retire the orphan and its exact-owned process routes under the shared state lock without signaling stored PIDs. Ordinary launch must revalidate exact evidence under that lock, preserve unrelated routes and sessions, and report successful recovery even if later startup fails. The stop-only `--forget-ambiguous-orphans` repair may relax pending or legacy-untracked spawn evidence for a dead supervisor, but it must still refuse every live or uncertain registered identity, must never signal persisted PIDs, and must not be used by launch or replacement.
- Contextless `dev status --all`, `dev status --session ID`, `dev recover --session ID`, and `dev stop --session ID` use the selected state directory and persisted exact identity; they never canonicalize a saved repository root. Recovery is strict metadata-only. Exact stop may use authenticated supervisor control or the explicit stop-only ambiguity override.
