...
- Windows foreground cleanup owns each suspended child in a kill-on-close Job Object before resume, requests a targeted CTRL+BREAK for its new process group, falls back to non-forced tree termination when console delivery is unavailable, and uses Job termination as the bounded force path. Native executable targets may retain canonical/verbatim spelling, but batch shims and the working directory must remain representable through their legacy Windows boundaries. BSD-like process cleanup remains best-effort. Do not publish process-owned routes on platforms without high-confidence process start-token support; use aliases or no-proxy mode there until platform-specific start tokens are implemented.
