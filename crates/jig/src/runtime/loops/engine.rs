...
use super::workflow::{
    ...
    TuningOverrides, UnexecutedReason, WorkflowCompletion, WorkflowOutcome, WorkflowTick,
...
fn tick_with_execution(
    ...
    let mut lease_store = LeaseStore::for_workflow_claim(ctx, &workflow, &|| observer.cancelled())?;
    ...
    if !workflow.enabled {
        ...
        match lease_store.acquire_with_cancellation(
            &lease_key,
