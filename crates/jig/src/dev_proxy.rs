...
pub(crate) mod commands {
    ...
    pub(crate) fn proxy_without_context(command: ProxyCommand) -> Result<Value> {
        match command {
            ProxyCommand::Start(opts) if opts.foreground => {
                let mut settings = settings_without_context(&opts.proxy)?;
                settings.additional_dns_names = opts.certificate_dns_name;
                jig_dev_proxy::proxy_start(jig_dev_proxy::ProxyStartRequest::new(settings, true))
            }
    ...
    fn proxy_start(ctx: &RepoContext, opts: ProxyStartRequest) -> Result<Value> {
        let mut settings = settings(ctx, &opts.proxy)?;
        settings
            .additional_dns_names
            .extend(opts.certificate_dns_name);
