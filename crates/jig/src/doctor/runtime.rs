...
enum RuntimeDiagnosis<'a> {
    LauncherUnreadable(&'a str),
    LegacyLauncher(&'a str),
    LauncherMissing,
    InstallerUnreadable(&'a str),
    InstallerMissing,
    ContractUnreadable,
    UnsupportedContract(u32),
    LauncherProtocolMissing,
    InstallerProtocolMissing,
    ContractMismatch { actual: Option<u32> },
    Compatible { repair_seeded: bool },
}

impl RuntimeDiagnosis<'_> {
    ...
    const fn status(self) -> &'static str {
        match self {
            ...
            Self::LegacyLauncher(_) => "migration needed",
            Self::LauncherUnreadable(_)
            | Self::InstallerUnreadable(_)
            | Self::ContractUnreadable => "unreadable",
            Self::LauncherMissing | Self::InstallerMissing => "missing",
            Self::UnsupportedContract(_) => "unsupported",
            Self::LauncherProtocolMissing
            | Self::InstallerProtocolMissing
            | Self::ContractMismatch { .. } => "outdated",
        }
    }

    fn detail(self, current_version: &str, contract_label: &str) -> String {
        match self {
            Self::LauncherUnreadable(error) => {
                format!("running {current_version}, scripts/jig is unreadable ({error})")
            }
            Self::LegacyLauncher(version) => format!(
                "running {current_version} for contract {contract_label}, but scripts/jig still pins legacy product version {version}"
            ),
            Self::LauncherMissing => {
                format!("running {current_version}, but scripts/jig is missing")
            }
            Self::InstallerUnreadable(error) => {
            ...
            Self::ContractMismatch { actual: None } => format!(
...
struct RuntimeFacts<'a> {
    launcher: &'a LauncherVersion,
    installer: &'a InstallerVersion,
...
impl<'a> RuntimeFacts<'a> {
    fn diagnosis(&self) -> RuntimeDiagnosis<'a> {
        if let Some(error) = self.launcher.read_error.as_deref() {
            RuntimeDiagnosis::LauncherUnreadable(error)
        } else if let Some(version) = self.launcher.version.as_deref() {
            RuntimeDiagnosis::LegacyLauncher(version)
        } else if !self.launcher_present {
            RuntimeDiagnosis::LauncherMissing
        } else if let Some(error) = self.installer.read_error.as_deref() {
            RuntimeDiagnosis::InstallerUnreadable(error)
        } else if !self.installer_present {
            RuntimeDiagnosis::InstallerMissing
        } else if self.contract_version.is_none() {
            ...
            .filter(|version| !crate::context::is_supported_contract_version(*version))
        {
            ...
            RuntimeDiagnosis::LauncherProtocolMissing
        } else if !self.installer.contract_probe {
            RuntimeDiagnosis::InstallerProtocolMissing
        } else if self.contract_version != self.launcher.contract_version {
            RuntimeDiagnosis::ContractMismatch {
                actual: self.launcher.contract_version,
            }
        } else {
            RuntimeDiagnosis::Compatible {
...
pub(super) fn runtime_check(
    ...
    let launcher_present = script_path.is_file();
    ...
    let facts = RuntimeFacts {
        ...
        installer_present,
        contract_version,
        repair_seeded: launcher_repair_seeded_cache,
    };
    let diagnosis = facts.diagnosis();
    let ok = diagnosis.is_compatible();
    let contract_label = contract_version.map_or_else(|| "unknown".into(), |v| v.to_string());
    let detail = diagnosis.detail(current_version, &contract_label);
    ...
    let fix = if !ok {
        ...
        if let RuntimeDiagnosis::UnsupportedContract(version) = diagnosis {
        ...
        } else if diagnosis == RuntimeDiagnosis::ContractUnreadable {
            ...
            && (!launcher_present || !installer_present || !narrow_repair_recognizable)
        {
            Some(format!(
    ...
    check("runtime", "Runtime compatibility", true, ok, status, detail)
        ...
        .with_data(json!({
                ...
                "legacy_launcher_version": launcher.version.as_deref(),
                "launcher_version": launcher.version.as_deref(),
