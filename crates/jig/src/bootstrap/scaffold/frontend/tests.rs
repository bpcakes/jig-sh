...
use super::templates::{
    ...
    FRONTEND_WORKSPACE_TEMPLATES, GO_PUBLIC_API_CLIENT_CONTRACT_TEMPLATES, PNPM_WORKSPACE_TEMPLATE,
    PUBLIC_API_CLIENT_SHARED_TEMPLATES, REACT_ESLINT_TEMPLATE,
    RUST_PUBLIC_API_CLIENT_CONTRACT_TEMPLATES, SPA_SHADCN_TEMPLATES, VITE_REACT_TEMPLATES,
...
fn e2e_workflow_paths_have_one_role_and_database_aware_authority() {
    ...
    for authority in [
        ...
        assert!(
            go.iter().any(|path| path == authority),
            "missing {authority}"
        );
    }
}
