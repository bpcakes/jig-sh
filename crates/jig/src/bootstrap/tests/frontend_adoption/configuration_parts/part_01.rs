...
fn init_renders_web_commands_for_all_supported_package_managers() {
    ...
    for (package_manager, install_command, run_command) in cases {
        ...
        assert!(agent_guidance.contains(
            "Run package-local scripts through `scripts/check-webapps.sh run-script <app-dir> <script>`"
        ));
        assert!(agent_guidance.contains("Iteration results do not replace final requirements"));
        assert!(
            !agent_guidance
