...
fn protect_explicit(records: Option<&[Value]>, root_ids: &[&str]) -> Result<BTreeSet<String>> {
    let temp = tempdir()?;
    let path = temp.path().join("receipts.jsonl");
    ...
    let roots = root_ids.iter().map(|id| (*id).to_string()).collect();
    let mut protected = BTreeSet::new();
    with_jsonl_write_lock(&path, |guard| {
        protect_receipt_ids(guard, &path, &roots, &mut protected)
    })?;
    Ok(protected)
}
...
#[test]
fn expired_explicit_tracker_root_still_pins_its_dependency_proof() {
    let mut root = receipt(0, Some(1));
    root["target_freshness"]["effective_valid_until_ms"] = json!(99);
    root["target_freshness"]["effective_requires_time_validity"] = json!(true);

    let protected =
        protect_explicit(Some(&[root, receipt(1, None)]), &["receipt_example_0"]).unwrap();
...
#[test]
fn missing_receipt_journal_has_no_explicit_bytes_to_protect() {
    let protected = protect_explicit(None, &["receipt_missing"]).unwrap();
    assert!(protected.is_empty());
}

#[test]
fn pending_plan_uses_archive_selection_and_preserves_target_dependencies() {
    let temp = tempdir().unwrap();
    let path = temp.path().join("receipts.jsonl");
    let mut root = receipt(0, Some(1));
    root["plan_id"] = json!("plan_tracker");
    let mut dependency = receipt(1, None);
    dependency["plan_id"] = json!("plan_dependency");
    let mut unrelated = receipt(2, None);
    unrelated["plan_id"] = json!("plan_tracker");
    let bytes = [&dependency, &root, &unrelated]
        .into_iter()
        .map(|record| format!("{record}\n"))
        .collect::<String>();
    fs::write(&path, bytes).unwrap();
    let configured = super::super::ConfiguredGateEvidence {
        check_tools: BTreeSet::new(),
        check_gate_ids: BTreeSet::new(),
        review_gate_ids: BTreeSet::new(),
        targets: BTreeMap::from([(
            "verify".into(),
            BTreeSet::from(["example:check-0".parse().unwrap()]),
        )]),
    };
    let protected_plan_ids = BTreeSet::from(["plan_tracker".into()]);

    let protected = with_jsonl_write_lock(&path, |guard| {
        super::super::plan_receipt_dependency_closure(
            8,
            guard,
            &path,
            &protected_plan_ids,
