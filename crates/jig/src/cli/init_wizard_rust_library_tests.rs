...
#[test]
fn explicit_rust_library_interactive_mode_prompts_for_nothing() {
    let mut opts = init_opts(&[
        "jig",
        "init",
        "ExampleLibrary",
...
#[test]
fn rust_library_rejects_misspelled_dev_answer_fields() {
    let temp = tempdir().unwrap();
    let answers = temp.path().join("answers.toml");
...
#[test]
fn bootstrap_guard_rejects_forbidden_rust_library_input_without_the_wizard() {
    let temp = tempdir().unwrap();
    let answers = temp.path().join("answers.toml");
...
#[test]
fn existing_preset_init_still_rejects_invalid_authored_command_maps() {
    let temp = tempdir().unwrap();
    let answers = temp.path().join("answers.toml");
    fs::write(
        &answers,
...
"#,
    ...
    .unwrap();
    let destination = temp.path().join("ExampleApplication");
    let mut opts = init_opts(&[
        "jig",
        "init",
        destination.to_str().unwrap(),
