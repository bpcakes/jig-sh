...
fn serve_messages(
    ...
    loop {
        ...
        // The complete frame has been consumed, so a rejected payload cannot
        // desynchronize the next request. Framing and I/O failures remain fatal.
        let message = match crate::strict_json::from_slice(&body) {
            Ok(message) => message,
            Err(_) => {
                write_message(
                    writer,
...
fn read_frame(reader: &mut dyn BufRead) -> Result<Option<(Vec<u8>, MessageFraming)>> {
    ...
    Ok(Some((body, MessageFraming::ContentLength)))
}
