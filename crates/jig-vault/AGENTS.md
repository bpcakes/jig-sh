...
- Backup envelopes keep vault and audit bytes encrypted, and restore installs only into a proven absent home through private sibling staging. Restore may prepare a missing private parent chain after rejecting existing symlink ancestors and validating the first existing creation boundary. Create, verify, and sync each missing component plus its containing entry independently of `umask`; reject group- or other-writable boundaries unless sticky and owned by the current user or root. Relative restore paths remain working-directory-relative, leading parent traversal requires an existing prefix, and verified empty parents may persist after later failure, but no restore path may resolve, create, or overwrite the target vault home before atomic installation.
