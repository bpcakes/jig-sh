...
`--patch` to preview paired authoring/manifest changes. Generated checks retain
Git freshness, including Cargo formatters whose command resolution can change
through aliases. After auditing Git independence, use an explicit
`--target component:action --assert-worktree` to opt into reuse across staging
and commits of checked bytes. Ordinary updates preserve owner-declared policies.
For repositories generated by the earlier unreleased formatter inference,
`jig update` (including `--recopy`) restores Git freshness when the saved policy
is `worktree` with `inferred` provenance and the command still matches the old
Cargo formatter shape. Changed runners require owner review.
...
If an older repository selected its schema output with the ambient
`SCHEMA_DOCS_DIR` variable, copy that repository-relative value into
`schema_docs_dir` in `.jig.toml` before readoption. Current Jig versions ignore
the environment variable so the committed configuration and gate scope cannot
diverge by machine.

Contract-v5 readoption replaces the old generated `contract` gate ID with `jig-contract`, retires the four blanket TypeScript work gates and the former mutating `schema-dump` work gate, and emits path-aware Rust/SQLx plus atomic per-app frontend gates. Existing repository-wide TypeScript tools and the explicit `scripts/jig sqlx schema dump` utility stay available for deliberate use. Removing a configured frontend app also retires only that app's exact unchanged generated command entries; modified same-key commands remain project-owned. Reconciliation preserves explicit project policy for `required` and `reuse` only when both an exact generated gate ID and its execution tool match. An exact identity always wins over a legacy alias when both are present; the retired alias is discarded instead of overriding current policy by file order. Generated `paths` and `paths_ignore` remain migration-owned and refresh from the current template so fixes to execution scope reach adopted repositories; define a distinct project-owned gate ID for a custom scope. Preserved custom commands on generated tool IDs do not inherit an unsafe narrow proof: unless the command still has the strict canonical Cargo, SQLx, or generated web-checker grammar, runtime applicability becomes conservatively unconditional. Adoption persists every supported non-app JavaScript workspace member after exclusions as `frontend_workspace_roots`, so arbitrary shared `libs/*`-style ownership and packages without dev scripts remain gate authorities. Other-app ignores are retained only for disjoint trees, so nested apps and shared roots are conservatively selected. Readoption also persists `schema_docs_dir` (default `docs/schema`) and refreshes Rust migration, Cargo/toolchain SQLx, and application/public-doc authorities. Schema destinations at the repository root, inside any case/HFS alias of `.git` or `.agent`, or ignored by repository policy must be moved or unignored before freshness checking. Use a distinct project-owned tool and gate only when the repository explicitly owns both the wrapper and its complete narrow input policy. An exact generated ID owned by another tool is a blocking collision that must be renamed explicitly. The only legacy aliases are `contract` to `jig-contract` for `jig.contract_check` and `tests` to `rust-tests` for `jig.test`. Project-owned gates are never consumed merely because they use the same tool. When adopted frontend apps have `scripts/contracts.mjs` with the exact `// jig-application-contract-checker: v1 modes=check,public-check` marker in its first eight lines, Jig infers `application_contracts_enabled = true` and generates separate application-contract freshness and public-artifact boundary gates. An unrelated or partial script leaves the capability disabled with a warning; owners of an equivalent checker may opt in explicitly with `--application-contracts-enabled true`. Those commands prepare the root dependency scope plus every distinct configured app scope, rather than assuming the first app owns the checker. Review the conservative generated SQLx/Rust scopes and shared frontend inputs against repository ownership, then open a new work plan so it captures a v5 Git baseline; path-aware gates intentionally remain unknown on a legacy plan.
