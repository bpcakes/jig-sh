...
**Unreleased change after 0.3.0:** current `master` removes external status providers and rejects the `[status]` and `[[status.providers]]` tables as unknown configuration. Remove those tables when upgrading to this source version. The published 0.3.0 release still supports them; see its [configuration guide](https://github.com/bpcakes/jig-sh/blob/8629700b92cd9ab8b09f8ff86de4fc1573469c83/docs/configuration.md) for the released configuration surface.
...
Below that status line, the Codex picker also answers a collective question: at the estimated aggregate pace, with work moved freely between the participating accounts, does the modeled pool reach a period in which no account can accept work before quota becomes available again? The forecast counts distinct quota pools rather than home directories, always covers every discovered home rather than the current search results, and reports its own coverage: `All accounts 3/3` only when every discovered home contributed, `Fleet partial 2/3` with the excluded reasons named in the selected-home pane otherwise. The result is `no gap projected through <horizon>`, `gap risk in ~<time>; capacity returns in ~<time>`, `all accounts blocked`, `collecting`, or an explicit `Fleet unavailable` reason. A window still inside its warmup withholds a consumption forecast, but it never hides quota that is already gone: a pool with an exhausted window is still reported as blocked, with the reported reset that ends the block, and the detail pane notes that pace evidence is still collecting. A warming pace is never used to conclude that an account is exhausted, because that is the estimate warmup withholds. Samples taken at slightly different times are carried forward to the newest one, and an account's windows advance together: while any of its windows blocks work, none of its allowances is spent, so a blocked account keeps whatever quota it still had when its blocking window resets. Its estimated pace still counts toward fleet demand. It ages with the same 15-minute sample expiry as the per-account projection and then reads `fleet forecast stale · reopen to refresh`.

The selected-home pane lists the forecast's assumptions, and they are assumptions rather than provider guarantees. Pace is the same **window-average** estimate the per-account projection uses—reported usage divided by elapsed window time—so it can lag a recent burst. Accounts are pooled only when they report the same plan and the same window layout; mixed plans are refused as `mixed capacities need comparable quota weights`, and a plan that is missing or explicitly reported as unknown is excluded as `unknown plan capacity`, because the normalized inspection payload carries no absolute quota sizes to convert between capacities. Homes reporting the same account email are conservatively treated as one shared quota pool: the freshest sample governs, and if two homes disagree at the same observation second, that pool is withheld as `conflicting samples for one account` rather than decided by discovery order. Future resets repeat each reported window duration as a **periodic approximation**; only the next reset is server-reported. Each account keeps a separate budget per quota window, an account can serve work only while every one of its windows has quota left, and a reset replaces that one window's allowance instead of accumulating unused quota or refreshing a sibling. The horizon is one longest participating window duration, and a very large discovered-home list can end in a stated forecast budget limit instead of a result. A gap is risk under this documented scenario—earliest reset spent first—and not proof that no allocation could avoid one. Nothing here promises that a running session migrates between homes, or that provider, model, credit, or concurrency limits are satisfied.
