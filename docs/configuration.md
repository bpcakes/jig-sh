...
Managed language test workflows invoke legacy selectors such as `fmt`, `lint`, `clippy`, and `test-locked`. Recopy retains a language workflow for an authored contract-v6 model only when its actions declare every compatibility alias that workflow invokes; an adapter alone does not imply those scaffold conventions. Repository-policy jobs that use native adapter policy remain independently composable.
