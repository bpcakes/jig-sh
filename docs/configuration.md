...
An incompatible live proxy is rejected before HTTPS certificate preparation changes shared certificate files. A temporary capability-probe failure asks for a retry during startup; while a development session is running, it counts toward the existing bounded proxy-health miss budget.
