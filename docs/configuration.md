...
- `schema_docs_dir`: normalized committed repository-relative output directory checked by schema freshness; defaults to `docs/schema`, may not be the repository root, may not use pathspec syntax or contain a case/HFS alias of `.agent` or `.git`, must not be ignored by Git, and is rendered into schema-gate applicability. The check explicitly reports all untracked and ignored output regardless of ambient `status.showUntrackedFiles` configuration.
...
`SCHEMA_DOCS_DIR` is no longer read from the ambient environment. Repositories
that previously used it must persist the same repository-relative destination
as `schema_docs_dir` in `.jig.toml`; otherwise Jig deliberately uses
`docs/schema`. This committed value is the authority for both generation and
freshness evidence.
- `frontend_workspace_roots`: normalized non-app JavaScript workspace members discovered during adoption; their complete trees are shared execution authorities for generated frontend gates
...
Top-level `*_command` values are committed repo configuration and run through non-login `bash -c` from the repo root with the user's normal process environment. They run in supervised process trees, use `[execution].command_timeout_seconds` (default 1,800; valid range 1–86,400), and retain at most `[execution].command_output_limit_bytes` from each stdout/stderr stream (default 67,108,864; valid range 1–1,073,741,824). Exceeding that authoritative capture limit terminates and reaps the owned process tree immediately as an explicit failed execution; it is never reported as a partial success or allowed to run until the timeout. Internal Git and GitHub protocol commands keep a separate fixed 4 MiB bound. Codex review, refinement, and PR-repair workers similarly use a separately bounded `-o` last-message file as their authoritative result channel, whether or not an output schema is configured. Their diagnostic provider stdout/stderr transcripts may truncate at 4 MiB without discarding a valid result, and worker receipt evidence reports `stdout_truncated` and `stderr_truncated`. Human-mode CLI phase, output-preview, and heartbeat events are collected in a bounded 64 KiB buffer while the child is supervised, then receive a deadline-bounded best-effort stderr delivery after supervised execution returns and before any restored terminating signal is redelivered. This prevents a stalled stderr consumer from suspending timeout or cancellation enforcement; if the sink remains stalled at finalization, Jig drops the remaining human preview and suppresses later stderr writes so command completion and signal retirement can continue. JSON mode keeps progress disabled so its stdout and stderr contract remains clean. MCP progress similarly defers transport writes until execution returns, retains at most a 4 KiB preview per stream, and emits at most one output notification for each stream. Treat changes to command keys like changes to project-owned shell scripts. Jig-owned Bash probes are narrower: frontend dependency readiness and launcher-backed doctor proxy diagnostics remove inherited Bash startup files, directory lookup, shell-option/trace controls, and exported functions before execution so those controls cannot spoof or corrupt structured results. Ordinary configured checks and development commands retain the user's environment. Jig-owned checks such as `scripts/jig check contract`, flat-layout `scripts/jig sqlx migration add NAME`, `scripts/jig check schema`, and repo policy checks run natively inside the binary.
...
- `[[work.gates]]`: `id`, `kind`, `tool`, `skill`, `fail_on`, `severity`, `scope`, `model`, `required`, `paths`, `paths_ignore`, `reuse`
...
`schedule` uses minute, hour, day-of-month, month, and day-of-week fields; seconds, years, and expressions with no possible calendar occurrence are rejected. `timezone` is an IANA name and defaults to `UTC`. A nonexistent spring-forward wall time is skipped, while a repeated fall-back wall time runs once. A `codex_task` requires both `schedule` and a repository-relative `prompt_file`. Prompt symlinks must still resolve inside the repository and prompt files are limited to 1 MiB of UTF-8 text. `sandbox` defaults to `read-only` and permits only `read-only` or `workspace-write`; `checkout` defaults to an isolated detached `worktree` and may be explicitly set to `repo`. `model` is optional.

Manually validate a configured task with `scripts/jig loop tick --workflow nightly-maintenance`. Scheduled `codex_task` workflows deliberately do not support `loop run`, because repeated reconcile ticks would execute the same task more than once; use `loop tick` for one manual execution or `loop dispatch` for its durable schedule. Run `scripts/jig loop dispatch` every minute from cron, systemd, launchd, or CI to execute due occurrences. On the first dispatch, the most recent scheduled instant is due; after that, missed intervals coalesce to the most recent unclaimed occurrence. Durable claims under ignored `.agent/runtime/loop/` prevent duplicate execution in a persistent checkout; preserve that directory when a scheduler recreates or deploys the checkout. The occurrence ledger is deliberately separate from disposable `.agent/.cache/loop/` leases, attempts, and worktrees. Current Jig migrates a legacy cache ledger before dispatch and leaves a fail-closed marker for older runtimes; stop older Jig dispatchers during the upgrade.

Losing a workflow or occurrence lease terminates its worker, and an expired running claim becomes `needs_attention` rather than being retried automatically because its side effects may be ambiguous. Outstanding `needs_attention` occurrences are never history-pruned and keep later dispatch results unsuccessful, so external schedulers continue surfacing the unresolved state. After inspecting the receipt and any retained worktree, run `scripts/jig loop acknowledge-occurrence --occurrence <reported-id>`. Acknowledgement is an explicit terminal transition: it clears the alert but keeps the schedule instant recorded, so the task cannot rerun. One workflow's state failure is reported without preventing other due workflows from running. `loop run` continues after workflow-reported failed actions and preserves an unsuccessful aggregate result; configuration, persistence, receipt, and other engine-level errors abort the command. Because every `checkout = "repo"` task mutates the same repository root, repo-mode tasks share one repository-local execution lease even when their workflow IDs differ; isolated worktree tasks retain per-workflow leases. A clean successful worktree is removed, while a worktree with uncommitted changes, new commits, or a failed worker is retained and reported by `loop status` and `jig ui`. Retained-worktree occurrence records are excluded from bounded history pruning while their paths exist. After preserving or discarding the result, remove a clean retained checkout with `git worktree remove <reported-path>`; Git refuses a dirty checkout unless you deliberately add `--force`. Once the path is gone, a later loop state update can prune the old occurrence normally. The external scheduler and machine must be running for local execution; Jig does not install or run a resident scheduler.

`max_attempts` and `backoff_seconds` apply to workflows that maintain per-item attempt records, currently `pr_manager`. Each `codex_task` schedule occurrence is claimed and run at most once; a failed occurrence is recorded for inspection and is not retried automatically. `loop tick`, `loop dispatch`, and `loop run` return a nonzero process status when their JSON result reports `ok: false`, so external schedulers can alert on failed runs.
...
paths_ignore = ["crates/**/fixtures/**"]
reuse = false
```

`kind: check` gates must reference no-argument execution tools declared in `.agent/jig-contract.json`. A missing `required` value defaults to `true`. A missing `paths` value means the gate is always applicable. When `paths` is present it must be a nonempty list of repository-relative globs; `paths_ignore` is allowed only with `paths`, and matching ignores win except for Jig's global execution authorities. Patterns use `/` separators and support `*`, `?`, character classes, and `**` as a complete path component. A bare directory pattern such as `docs` and a trailing `docs/` match only what this grammar selects; use `docs/**` for its descendants. Brace expansion is deliberately unsupported; write separate patterns instead. Absolute paths, `.` or `..` components, backslashes, braces, ambiguous mixed `**` components, control characters, and `.agent/**` are rejected because applicability must use one deterministic, repository-bounded grammar independent of append-only Jig state. Git fingerprinting receives only the classifier's exact selected repository paths, so it cannot reinterpret those globs. Generated narrow policy for Jig's known Rust, SQLx, schema-check, application-contract, public-artifact, and per-app TypeScript tool IDs is used only while the resolved command has its strict canonical grammar. Environment wrappers, path-selection flags, cross-app tool/command mismatches, and unfamiliar command shapes make a generated gate unconditional. Native tools are intrinsically known except `jig.schema_check`, whose scope also depends on the project-owned schema dump command it invokes. Define a distinct project-owned tool and gate when a custom command has an explicitly owned narrow input contract.

`scripts/jig work start` resolves `HEAD` to an exact commit and stores that immutable plan baseline. In an unborn repository it records and materializes Git's hash-format-specific empty tree instead, so every initial file is classified relative to an empty repository. Pass `--base REF` to select another commit; an invalid explicit ref fails before plan state is written. Applicability compares that object with the current index and worktree, including commits made after plan start, staged and unstaged edits, deletions, both sides of renames, and untracked paths. A path-aware gate on a legacy plan without a baseline reports `unknown` and blocks closure; reopen the work under a current plan. Append-only `.agent/**` state never participates, but `.agent/jig-contract.json` and `.jig.toml` are non-ignorable global execution authorities: changing either makes every path-aware check gate applicable.

With no selector, `scripts/jig work check --plan-id ...` runs required applicable check gates in configuration order. Optional gates do not run by default. This required-only default belongs to contract v5; contracts v2-v4 retain their historical all-configured-gates default. A non-applicable gate records explicit `not_applicable` classification evidence and no child tool receipt. Pass `--gate ID` to force-run a configured gate, including an optional or currently non-applicable gate. When Git classification is unknown, forcing a gate still executes its tool but deliberately leaves applicability and closure status unknown. Under contract v5, the legacy `--tool TOOL` selector force-runs every configured gate using that tool, or runs the tool directly when no gate references it. Contracts v2-v4 preserve their historical direct-tool behavior, including caller order and duplicate selectors. `--gate` and `--tool` cannot be combined. Human-readable output is the default; pass `--json` for the full baseline, classification, matching-path, signature, fingerprint, and source-receipt evidence.

Every work-check batch names all selected gate IDs. If execution fails or is
cancelled, the batch records `failed` or `cancelled` evidence for the active
gate and `unknown` interruption evidence for every selected gate that did not
complete. This newer batch supersedes older evidence for those gates, so a
previous pass cannot remain authoritative after an interrupted rerun.

`reuse = true` is an explicit repository policy for deterministic gates. Jig may skip execution only when it finds direct successful `executed` evidence from a different plan with the exact same gate ID, execution/policy signature, baseline-relative scoped fingerprint, successful proving tool receipt, and a clean source batch whose whole-worktree before/after fingerprint remained stable. Reused attestations are never chained. It appends a new plan-scoped `reused` attestation that references the original source evidence; after any evidence exists for that gate in the current plan, another `work check` executes it instead of alternating back to external reuse. Reuse defaults to false and never applies to Codex review gates.

`scripts/jig work gates --plan-id ...` reports each configured gate as `passed`, `reused`, `not_applicable`, `missing`, `failed`, `stale`, `unknown`, or `unsupported`. A fresh `passed`, `reused`, or `not_applicable` result satisfies a required gate. Executed and failed gate evidence carries the child tool's `exit_status`; this remains truthful even when a refinement batch deliberately aggregates failures and exits successfully. Pass `--json` when automation needs the full structured payload. `scripts/jig work evidence` is the higher-level human view. For `work gates` and `work evidence`, top-level `ok: true` means inspection completed; read `overall`, `gates_ok`, and each gate `status` to decide whether work is blocked. Changed and matching path lists are bounded previews with full-list counts and digests and can contain local filenames, so receipt JSON is not secret-free metadata. `scripts/jig work finish --plan-id ...` refuses to close work while required gates are missing, failed, stale, unknown, or unsupported.

Path-aware check freshness is scoped to the gate rather than the entire dirty tree. Classification is the union of baseline-to-index and baseline-to-worktree changes, and the fingerprint covers both canonical diff domains over only the classifier's literal selected tracked paths plus matching untracked file type, executable mode, metadata, and contents. A path changed in the index whose worktree copy also differs from the index is partially staged and fails scope proof as unknown; ordinary unstaged-only and staged-only inputs remain supported. Tracked executable-mode discovery is forced on even when ambient `core.fileMode` is false; external diff commands and text conversions are disabled, and diff context, submodule treatment, inter-hunk context, relative prefixes, ordering, renames, algorithm, and indentation behavior are pinned. A selected initialized gitlink must match its index object and have a recursively clean checkout; the recursive cleanliness probe consumes only the first status byte, while dirty or advanced-but-unstaged submodules and untracked embedded repositories make applicability unknown because a parent gitlink/directory marker cannot attest their contents. Selected tracked paths are transported in deterministic bounded chunks so a large affected set cannot exceed the host argument limit. Changed-path Git name output is capped at 16 MiB per command, every scoped or whole-worktree binary diff at 64 MiB, and aggregate discovery at 250,000 entries; exceeding a bound reports unknown and requires splitting or reducing the worktree change set. An unrelated edit does not stale the evidence; an in-scope edit, rename, deletion, mode change, command or policy change, or baseline change does. Native-tool signatures also contain Jig's build-source identity, including source inputs, target/host, Cargo cfgs and features, profile/codegen settings, compiler path and verbose version, wrapper/encoded flags, package metadata, embedded-template mode, and final template-pin policy. Checkout mode requires Jig-specific workspace sentinels, while registry/path packages use only complete package-local inputs. Within one command, Jig discovers baseline-relative changed paths once, streams each unique normalized gate-policy diff through bounded memory, caches that scoped input, and scans receipt history once for all reusable gates. Multi-plan status reads plan baselines once, shares one plan-change snapshot and normalized-policy proof for plans with the same baseline, and shares its current whole-worktree fingerprint; legacy unconditional gates also share that request-scoped value. Plan-wide changed-path preview/count/digest fields appear once on the work-check batch evidence, while each gate keeps only its matching paths and policy proof; older per-gate records remain readable. Work check starts its stable-worktree interval before scope discovery, independently re-evaluates every selected scope after execution, and closes the interval afterward; a changed scope or a check that mutates non-`.agent/` files cannot produce fresh evidence.
...
After upgrading an in-flight repo from an earlier contract epoch, open a new plan to capture a baseline and rerun `scripts/jig work check --plan-id ...`. Older successful check receipts deserialize correctly. Unconditional gates retain their legacy whole-worktree fallback, while path-aware gates require current gate-ID classification evidence. Contracts v2-v4 reject the v5-only `paths`, `paths_ignore`, and `reuse` fields instead of accepting policy they cannot represent with scoped evidence.
...
Contract-v5 full harnesses generate path-aware Rust format, Clippy, and test gates. Their scopes include the corresponding `rustfmt.toml`, `clippy.toml`, and Nextest configuration authorities in addition to Rust source, Cargo manifests and locks, toolchain files, and `.cargo/**` inputs; Rust tests also own configured migrations because scaffolded code may embed them. SQLx-enabled repos include a conservative path-aware `jig.sqlx_check` gate over Rust, migration, metadata, SQL, Cargo/toolchain, and `.cargo/**` inputs; owners should narrow it with a distinct project-owned gate when database-query ownership is more precise. Repos with schema dumps enabled include one validation gate, `jig.schema_check`, over Rust/toolchain, migration, metadata, SQL, and the committed `schema_docs_dir`. `jig.schema_dump` remains an explicit mutating utility and is never generated as a required work gate; readoption retires the former generated `schema-dump` gate. Each configured frontend app receives four atomic app-specific lint, typecheck, build, and coverage tools/gates; app paths select only that app, while root and nested package-manager configuration, manifests and locks, workspace declarations, Node version files, Yarn patches/plugins/releases, package patches, inferred non-app workspace member trees (including members without dev scripts), root TypeScript/lint/build/test configuration, and generated frontend helper scripts select every app. When readoption removes an app, it also retires that app's exact unchanged generated command entries; a customized command with the same key remains project-owned. Workspace exclusions are applied before ownership is persisted. Other-app ignores are emitted only for disjoint trees, so nested app or shared workspace ownership is conservatively included rather than masked. Application-contract and public-artifact commands prepare the root package boundary when present and every distinct dependency scope represented by configured apps before invoking the root checker. Their gates include those execution authorities, every configured app tree, and conventional `docs/public/**` plus `public-docs/**` inputs so handwritten transport types, generated clients, public docs, and feature-owned parsers cannot bypass compatibility validation. Public-artifact gates include the same public-doc inputs, authorities, and app manifests while excluding privileged app source from public builds. Generated scope fields are migration-owned during readoption; `required` and `reuse` remain project policy. The repository-wide TypeScript check commands remain available for deliberate full-matrix validation.

The generated `jig-contract` gate is deliberately named for the harness. `jig.contract_check` validates Jig's generated manifest, configured command keys, native tool availability, and runtime wiring. It does not validate an application's OpenAPI document or frontend/backend API compatibility. When `application_contracts_enabled = true`, the full frontend harness instead generates an explicit `application-contracts` gate that runs `scripts/contracts.mjs check`, plus a separate `public-artifacts` gate that builds every non-admin frontend and scans its output for privileged artifacts. The Rust/React scaffold owns and enables both; adopted repositories get them only when the checker already exists, and a missing enabled checker fails rather than passing as a no-op.
...
`scripts/jig work refine --plan-id ...` runs a review-driven fixer loop. It runs review gates, passes actionable findings to `codex --ask-for-approval never exec --sandbox workspace-write` for direct repository edits, reruns review gates, then reruns required applicable check gates. Enabling refinement opts into unattended Codex workspace writes: the prompt tells the fixer not to run git, but the sandbox still permits repository edits. Review skills used with refinement are trusted inputs because their finding text is handed to an auto-approved workspace-writing fixer; keep refinement-enabled review skills sourced from trusted Codex marketplaces or repos and review the resulting diff before closing work. Refinement requires one explicit `[[work.refinements]]` entry before Jig will invoke the workspace-writing fixer. Without a refinement `model`, the fixer uses the first selected review gate model when present. `--max-iterations` controls fixer attempts and defaults to 1, meaning Jig fixes once and then verifies. Passing `--gate` narrows only the review gates; the final verification step still applies the complete required check-gate policy. An optional `[[work.refinements]]` entry provides a repo-local refinement profile for the fixer prompt:
...
When `sqlx_enabled` is `true` and `rust_migration_layout` is `flat_migrations`, it also exposes:
...
`scripts/jig check schema` reruns `schema_dump_command`, then checks the committed `schema_docs_dir` for drift using literal Git path semantics. The directory defaults to `docs/schema`; it must be a dedicated subtree outside reserved state/metadata components. Known-root Git probes scrub ambient repository, worktree, index, object-store, namespace, replacement-ref, and command-scoped configuration redirects, so environment variables cannot redirect the validation target.

The legacy `scripts/jig migration-add NAME` and `scripts/jig schema-dump` paths remain accepted as compatibility shims, but new documentation and automation should use the `scripts/jig sqlx ...` namespace. Both migration-add paths reject `versioned_artifacts` repositories before creating files.
