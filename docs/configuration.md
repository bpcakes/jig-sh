...
Scaffold outputs are whole-file, project-owned artifacts. For Rust presets this includes `Cargo.toml`, member manifests and source, and root `clippy.toml`. Re-running `jig init` against differing ordinary files fails before publication; `--force` explicitly authorizes their complete replacement and reports them as modified. Use `jig adopt` when Cargo or Clippy authority already exists and must be preserved.
...
- `bootstrap_command`: implementation behind `scripts/jig bootstrap`; `scripts/jig setup` invokes it between an initial doctor pass and minimum agent/contract verification. The generated harness-only default runs `cargo fetch` only when a root `Cargo.toml` exists, otherwise exits 0 with a stdout note. Generated package `bootstrap` scripts call the same Jig command. Rust bootstrap fetches Cargo dependencies and installs frontend dependencies. Go bootstrap tidies modules, installs frontend dependencies, and prepares sqlc/API client code from local schemas. Neither generated bootstrap requires application database/provider credentials or initializes a database. Database initialization is explicit: run `bash scripts/setup-database.sh` (or the package `database:setup` script) after exporting a nonempty `DATABASE_URL` or configuring the backend `.env`. An empty or unrelated `.env` fails before starting the application. The script creates or reuses the database and applies SQLx or Goose migrations using the configured role's existing authority; it never creates roles or grants permissions. `test:postgres` independently proves migrations and generated queries against a disposable database. These commands and scripts are project-owned after init, so update/recopy preserve overrides; existing projects can move database initialization out of their authored bootstrap command explicitly. See [migrating existing database bootstrap commands](developer-ux.md#migrating-existing-database-bootstrap-commands) for the upgrade steps. If a root `Cargo.toml` exists, Cargo errors are surfaced instead of skipped.
...
or shell interpolation are supported. For example, within a `[[repository.actions]]`
entry:

```toml
[repository.actions.arguments.message]
type = "string"
required = true
max_bytes = 256

[repository.actions.arguments.note]
type = "string"
allow_empty = true
max_bytes = 1024
```
...
Git repositories keep the authoritative schedule ledger, initialization marker, and lock below the checkout's worktree-specific Git metadata directory. Codex `workspace-write` protects Git metadata. The protected ledger is the mutation commit point. Before creating protected authority, Jig durably refreshes the checkout-local recovery ledger and changes its initialization marker from public authority to a protected-cutover-pending state; it publishes the protected ledger and marker next, then finalizes the checkout witness before any protected mutation may run. A pending cutover resumes from that public recovery ledger only while Git authority is resolvable. Later reads interpret both pending and final witnesses independently of Git metadata and fail closed rather than promote the replica when protected authority is unavailable. After cutover, the checkout-local `.agent/runtime/loop/schedule.json` is only a compatibility and diagnostic replica: current runtimes ignore worker changes to it, retry its publication on later authoritative writes, and never let a ledger-replica publication failure discard or ambiguously fail an already committed protected transition.
...
`codex_home` is optional and valid for `pr_manager` and `codex_task`. It accepts the same conventional home names and explicit path forms as `scripts/jig codex launch`: `work` resolves only as `~/.codex-work`, `codex` and `default` resolve only as `~/.codex`, and explicit relative paths such as `./.codex-automation` resolve from the repository root. Use an explicit path for a home outside those conventional locations. Configured bare names never fall back to ambient `CODEX_HOME`. A repository-relative home is repository-controlled and may load Codex configuration such as MCP servers and model providers from tracked content; use one only in a repository you trust. Jig validates and canonicalizes the configured directory once at the workflow boundary, then sets `CODEX_HOME` explicitly for each `codex exec` worker. A missing or invalid home fails the tick. When `codex_home` is omitted, the worker preserves existing behavior by inheriting ambient `CODEX_HOME`. PR-repair worktree names use a digest of the workflow ID rather than the ID as a path component, so otherwise-valid IDs containing path separators cannot redirect worktree creation outside `.agent/runtime/loop/worktrees/prs/`.

After remote-head and occurrence reservation preflight, PR-manager retries remove and recreate any authenticated existing repair worktree instead of reusing ignored or nested content. Branch-derived worktree components use a bounded readable prefix plus a digest of the complete branch name. Merge and repair commits receive the Jig author identity only for that Git command, so the repository's configured `user.name` and `user.email` are not changed. Conflict repairs use Git's `AUTO_MERGE` tree as their whitespace-validation baseline. Immediately before commit, Jig recollects the complete bounded review-thread snapshot and retains the completed local repair without pushing if the PR head, actionable trusted-thread membership, reviewer authorization, or any other witnessed mutation authority differs from the worker snapshot. Review replies as well as resolutions are skipped if a complete live thread witness differs from the same worker snapshot.
...
The normalized GitHub observation has its own 16 MiB serialized-evidence limit and does not retain duplicate raw payloads. Each scheduled workflow writes detailed tick evidence once: dispatch output still expands the nested tick, while the dispatch receipt stores a typed reference to the tick receipt instead of copying the potentially large observation.

For a clean manual tick, the durable occurrence remains live through loop-tick receipt publication and stages available worker evidence before that write. Jig removes it only after the receipt commits. A receipt failure instead terminalizes the same occurrence as `needs_attention`, so another manual or scheduled run cannot repeat side effects across an evidence gap. If the staged record expires before finalization, stale reconciliation preserves its diagnostic while adding expiry context.

Loop execution fails closed at its mutable boundaries. Every Git-backed entrypoint that can publish schedule state verifies that `.agent/runtime/loop` is ignored before mutation, independent of workflow kind. Schedule storage plus retained task and PR worktrees reject symlinks in every managed path component, not only at the final directory. In Git repositories, lease ownership and retry-attempt budgets use the same worktree-specific protected Git-metadata directory as schedule authority, outside a repo-mode workspace-write worker's writable surface. The first mutation migrates any legacy `.agent/.cache/loop` record under both locks and replaces it with an older-runtime rejection marker; non-Git fixtures keep using the checkout cache through no-follow directory capabilities. A manual tick blocked by an occurrence requiring acknowledgement or by a retained worktree still writes its loop-tick receipt and returns a structured `needs_attention` result without starting the worker; overlap with a live occurrence similarly returns structured `waiting` evidence. Repo-mode Codex tasks require the shared checkout to be clean apart from the runtime-owned receipt journal before worker launch, so existing user work is never adopted as task output. Because that writable root contains every managed task and PR worktree, a shared-root occurrence is mutually exclusive with all live or unacknowledged workflow occurrences and with every retained worktree, including acknowledged evidence; isolated workflows resume only after the shared occurrence finishes, while a repo-mode workflow resumes only after retained paths are removed. PR-manager repair checkouts detach at the immutable head object observed in the GitHub snapshot. A PR head is writable only when GitHub explicitly reports `isCrossRepository = false` and the observed head repository's full owner/name identity equals the repository snapshot; missing, malformed, or mismatched metadata is skipped before branch lease acquisition. Preparation requires the remote ref to still equal that observed object, and publication requires both a descendant repair commit and an exact expected-head lease, so any concurrent branch advance, rewind, or deletion fails instead of redirecting or resurrecting history. Before committing, Jig requires Git's index to contain no unresolved merge entries, checks Git-defined whitespace errors only in worker-authored changes, and rejects conflict-marker diagnostics shared by both merge-parent comparisons. Marker examples inherited from either parent therefore remain valid while Git-introduced merge markers remain blocked. Review-thread comments are paged backward to a bounded safety limit so older trusted feedback remains visible. A truncated open-PR list invalidates the repository-wide candidate set, so `pr_manager` reports a failed action and performs no attempt cleanup or branch mutation until a complete list is available. A truncated review-thread connection or nested comment history instead makes only that PR ineligible and produces a retryable `waiting` action; other completely observed PRs remain eligible in the same tick. The complete observation shares one 256-request, 16-MiB-response, 10,000-review-item, ten-minute budget across repository metadata, PRs, checks, GraphQL pages, and collaborator permissions. Review-thread prechecks, mutations, and reconciliation separately share an aggregate deadline scaled by unique actionable worker intents up to ten minutes, plus per-intent command deadlines and request slices that prevent one thread from starving later intents. Known viewer capability denials are reported as skipped without issuing their impossible mutation. Budget exhaustion fails closed, and an already-incomplete comment history triggers no permission lookups.

Review text is an instruction boundary for the unattended PR-manager worker. Jig resolves each review-comment author's effective repository permission through GitHub's collaborator-permission API and treats only effective `admin` or `write` permission as trusted. Permission lookup failures fail closed. Untrusted unresolved threads remain observable in the GitHub status snapshot but do not make a PR repairable. The worker prompt receives an explicit projection that omits raw GitHub payloads, PR titles, and all untrusted comment bodies; only trusted unresolved comments can be replied to or resolved. Immediately before replying or resolving, Jig rechecks both the complete comment witness and the PR head returned with that thread; a head other than the pushed repair version skips the mutation as stale. The Codex sandbox remains an enforcement boundary, not a substitute for trusted prompt sources.
...
The `rust-react` preset always generates a Batter-based Rust service runtime, regardless of frontend or database selection. Lifecycle budgets and request admission are application-owned source policies, not `.jig.toml` settings; there is no legacy-runtime switch. See [Rust applications on Batter](rust-applications.md) for ownership, operational limits, and migration guidance.
...
When a proxy already runs in the selected state directory, these flags are reuse requirements. Jig verifies its effective LAN binding and HTTPS HTTP/2 setting through a versioned capability response that requires the private health token and loopback client/host, then checks the response against the live health PID and current runtime generation. Either LAN mismatch and either HTTP/2 mismatch when HTTPS is requested fail before app spawn or route publication; an HTTP-only caller may reuse a proxy that also serves HTTPS. A proxy too old to report capabilities also fails closed. The error names requested and observed settings or unknown evidence plus the state directory. Use matching flags, or explicitly stop and restart the shared proxy with that state directory and the desired settings after accounting for other sessions; Jig never restarts it for you. The existing health PID response remains available to older clients.

App-specific HTTPS certificate preparation follows authenticated compatible proxy readiness and rechecks that readiness under the certificate lock before writing. Inconclusive health with any remaining runtime record blocks a new daemon and preserves the records and certificates, even when a different or ephemeral port was requested. Retry when the proxy stabilizes, or inspect and explicitly stop the proxy in that state directory before starting again. A temporary capability-probe failure asks for a retry during startup; while a development session is running, it counts toward the existing bounded proxy-health miss budget.
...
On Linux and macOS, CLI `dev` launches keep the owning supervisor in a separate Unix session connected to the invoking CLI by a private lifetime socket. If the invoking CLI exits abruptly or its process group is killed, the worker observes socket closure and performs the same bounded preflight, app-tree, and route cleanup as SIGTERM. Normal terminal signals are forwarded to the worker, and the CLI preserves its output and exit status. The supervisor PID reported by `dev status` is the worker PID, not the invoking CLI PID. This protection applies to new CLI `dev` launches, including `--no-proxy`; direct library calls and `proxy run` retain their existing supervision. Killing the owning worker itself with SIGKILL, or killing both processes, can still leave an orphan. Existing sessions are not retrofitted by upgrading the executable.

For an orphan with a live app, `dev stop` and `dev --replace` report the surviving app PID and recovery guidance. Inspect `scripts/jig dev status --json`, independently verify and stop the remaining app processes, then retry `scripts/jig dev stop` or `scripts/jig dev --replace`. The stop-only `--forget-ambiguous-orphans` option cannot bypass live or uncertain process identities. A successfully retired orphan does not guarantee that replacement apps will start: an app's own initialization failure is reported separately, with the completed recovery retained in the report.

The proxy state directory also holds a private registry for active tied dev sessions. Records are keyed to the canonical repository root and carry supervisor/app observations, durable pre-spawn state, a durable preflight-cleanup obligation, an explicit cleanup-required marker, and an authenticated loopback control endpoint; `scripts/jig dev status` and `scripts/jig dev stop` expose only sanitized session data, never the control credential. Bare status is read-only and limited to the current repository. Bare stop targets all registered sessions for that repository, succeeds when none exist, and asks the live supervisor to use its retained child handles for cleanup. If the supervisor is gone while preflight cleanup is unconfirmed, an app spawn is pending or unknown, or a registered identity remains live or cannot be classified safely, Jig returns `ok: false` and retains the record without signaling persisted numeric PIDs. If cleanup evidence is complete and every exact registered identity is absent, status labels the session `recoverable`; ordinary same-repository `dev` launch, explicit `dev stop`, or `dev --replace` may then retire an overlapping dead orphan, removes only process routes still owned by its recorded PID/start-token pairs under the shared state lock, and emits a structured recovery notice that preserves app targets, spawn states, last-known PIDs, and explicitly forgotten ambiguities for diagnosis. Successful recoveries are separate from blocking warnings, and foreground `dev --replace` results retain recoveries completed before the replacement session was claimed. After independently confirming that no unrecorded process from interrupted preflight or spawn work remains, `scripts/jig dev stop --forget-ambiguous-orphans` may forget only dead-supervisor records blocked by unconfirmed preflight cleanup or pending or untracked spawn evidence. It still refuses any live or uncertain registered process identity, never signals a stored PID, and records that process absence could not be proved.

Bare `scripts/jig dev` automatically retires eligible dead overlapping claims from the same canonical repository without signaling persisted PIDs, preserving unrelated sessions and routes and reporting recovery even if later startup fails. `scripts/jig dev --replace` additionally resolves live conflicts by requesting authenticated shutdown of overlapping registered sessions from that repository, then atomically retrying the claim. It refuses cross-repository ownership, unregistered live process routes, and newly observed concurrent claims. This means a process launched by a pre-registry Jig version or by an ad-hoc command is never killed merely because its route hostname conflicts; stop that process explicitly and run `scripts/jig proxy prune` once.

When the original repository no longer exists, use `jig dev status --all --state-dir <path>` or `jig dev status --session ID --state-dir <path>` from any directory. These selectors read saved identities without opening the old repository, including no-proxy and legacy migration blockers. `jig dev recover --session ID --state-dir <path>` retires eligible metadata and exact-owned routes without stopping a process; `jig dev stop --session ID --state-dir <path>` can instead request authenticated shutdown of that exact live session. The stop-only ambiguity override remains explicit. A missing exact ID is a successful no-op, and no selector expands a prefix or wildcard.
