...
PR-manager worktrees live below `.agent/runtime/loop/worktrees/prs/`, alongside other durable retained evidence. Worktree preparation reports any partial or registered checkout as a cleanup candidate but never removes it directly. Before finalizing an outcome, Jig refreshes the branch lease; the shared cleanup boundary revalidates that exact owner again before every inspection and removal step and cancels in-flight Git cleanup if renewal loses ownership. A failed authority refresh, post-start cancellation, unconfirmed push, dirty or locally advanced failed checkout, or cleanup failure retains the checkout; the action reports `worktree_retained: true` and requires attention. Such side-effectful attention consumes the workflow tick, so Jig does not start another PR from the same snapshot. Acknowledgement clears the attention alert, but a retained PR worktree continues to block that workflow until the operator removes the reported path, matching isolated-task backpressure and bounding retained history. Passive `exhausted_attempt` attention may still allow another eligible PR to be considered.
