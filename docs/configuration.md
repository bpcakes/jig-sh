...
Manually validate a configured task with `scripts/jig loop tick --workflow nightly-maintenance`. Scheduled `codex_task` workflows deliberately do not support `loop run`, because repeated reconcile ticks would execute the same task more than once; use `loop tick` for one manual execution or `loop dispatch` for its durable schedule. Run `scripts/jig loop dispatch` every minute from cron, systemd, launchd, or CI to execute due occurrences. On the first dispatch, the most recent scheduled instant is due; after that, missed intervals coalesce to the most recent unclaimed occurrence. Successfully published claims are synced under ignored `.agent/runtime/loop/` before worker side effects begin, preventing duplicate execution after an abrupt host restart in a persistent checkout; preserve that directory when a scheduler recreates or deploys the checkout. The occurrence ledger and retained task worktrees are deliberately separate from disposable `.agent/.cache/loop/` leases and attempts. A retained worktree remains linked to the original Git checkout; copying or moving a checkout does not make those links portable by itself. When the main checkout and linked worktrees move together, run `git worktree repair` in the new location before relying on retained-worktree paths. Current Jig migrates legacy schema-1 and prior durable schema-2 ledgers to schema 3 before dispatch, leaves a schema-3 downgrade marker in the former cache location, and publishes `schedule.initialized` beside the durable ledger. The durable initialization marker remains authoritative after cache cleanup; either marker makes a missing ledger fail closed rather than defaulting to empty state. The legacy downgrade marker and schema-3 ledger make schema-2 runtimes fail closed, which is required because five-field cron occurrence identities are normalized to whole seconds in schema 3. Stop older Jig dispatchers during the upgrade and do not downgrade after a schema-3 dispatcher has written the ledger. If the durable target is lost after initialization, dispatch and status reads fail closed instead of creating an empty ledger: restore `.agent/runtime/loop/schedule.json` from durable storage or reconcile the legacy and durable files manually before running dispatch again.
