...
An `evidence` gate names exactly one canonical `target = "api:test"` or checked-in `profile = "verify"`. `conclusion` currently accepts only `success` and defaults to it. A target gate requires a successful receipt for that exact target. A profile gate requires current passing receipts for every current profile target and its execution prerequisites. The current runtime can validate original receipts from different runs when their freshness and dependency proofs remain valid; it does not substitute a pass from a different target.

Contract-v6 repositories generate a single gate for their default verification profile. `scripts/jig work check --plan-id ...` expands configured evidence gates to exact targets, reuses qualifying current passes, and executes targets needing new evidence with their prerequisites and invalidated dependents. It revalidates the required union and records a validation receipt referencing original target receipts linked to the work plan. Legacy `kind: check` gates remain supported and must reference no-argument execution tools declared in `.agent/jig-contract.json`; they run in configured order. When both forms exist, default `work check` runs both. Passing one or more `--tool` values explicitly selects only those legacy tools; their receipts cannot satisfy native target gates. Use `scripts/jig check COMPONENT:ACTION --plan-id ID` to force native target evidence. Human-readable output is the default. Pass `--json` for structured automation output.

Current passing evidence satisfies a check without a mandatory repeated final test command. Authored profiles and required review gates remain authoritative. Plain `check` executes selected targets on each invocation; `--affected` only narrows candidates and an affected no-op never proves all work gates passed. Supply the explicit plan ID for plan-linked checks, work checks, reviews, and finish. For blocked work, follow the existing recovery advice in `work gates` or `work evidence` before deciding to rerun checks.
