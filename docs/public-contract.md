...
Git repositories keep the authoritative schedule ledger, initialization marker, and lock in the checkout's worktree-specific Git metadata. This authority is outside a Codex `workspace-write` worker's writable surface. The checkout-local schedule ledger is a compatibility and diagnostic replica, so deleting or replacing it cannot erase occurrence history or permit the same occurrence to rerun; a later authoritative write restores the replica.
