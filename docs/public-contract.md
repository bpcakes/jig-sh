...
`scripts/jig work retire` ends a session only when durable state proves that session opened the retired plan, using the plan's `jig.plans_open` receipt. An unrelated current session is left active and the result's `session_status` object reports `action` (`ended`, `left_active`, or `none`), the proven `owner_session_id`, the `current_session_id`, and a human `detail`. The MCP tool `jig.work_retire` takes the same arguments and returns the same result shape.
