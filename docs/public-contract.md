...
Dev-session JSON is same-contract-epoch runtime output. `dev status` reports the canonical repo identity, resolved state directory, aggregate `running` state, and sanitized registered sessions with explicit process observations, spawn tracking, and a `recoverable` state for dead orphans. `dev stop` reports matched/stopped session and app counts, any sessions that remain, and warnings; it is successful and idempotent when no session matches, recovers an orphan once spawn state is known and every exact registered identity is absent, and returns `ok: false` when spawn state is pending or unknown or a registered process remains live or uncertain. `dev stop --forget-ambiguous-orphans` is an explicit repair for dead-supervisor records blocked only by pending or legacy-untracked spawn evidence; it still returns `ok: false` for live or uncertain registered identities and warns that an unrecorded process may remain. Neither response exposes the persisted session-control credential, and management never signals from persisted PID data.
