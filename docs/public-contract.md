...
Dev-session JSON is same-contract-epoch runtime output. `dev status` reports the canonical repo identity, resolved state directory, aggregate `running` state, and sanitized registered sessions with explicit process observations, spawn tracking, and a `recoverable` state for dead orphans. Aggregate `running` is true only when at least one session is neither stale nor recoverable; a recoverable record therefore leaves `running` false but remains in `sessions` until explicit cleanup, so callers deciding whether cleanup is needed must inspect the session list. `dev stop` reports matched/stopped session and app counts, any sessions that remain, blocking `warnings`, and structured successful `recoveries`; each recovery preserves diagnostic app names, targets, spawn states, and last-known PIDs after the registry entry is removed. It is successful and idempotent when no session matches, recovers an orphan once spawn state is known and every exact registered identity is absent, and returns `ok: false` when spawn state is pending or unknown or a registered process remains live or uncertain. `dev stop --forget-ambiguous-orphans` is an explicit repair for dead-supervisor records blocked only by pending or legacy-untracked spawn evidence; it still returns `ok: false` for live or uncertain registered identities and records that an unrecorded process may remain. Neither response exposes the persisted session-control credential, and management never signals from persisted PID data.
...
Runtime-owned `.jig.toml` sections are intentionally strict: unknown keys are rejected so local typos fail fast. New optional keys in `[work]`, `[loop]`, `[[loop.workflows]]`, `[status]`, `[[status.providers]]`, `[agent_tooling]`, `[agent_tooling.codex]`, `[dev]`, or app tables require a Jig runtime/template update and a documented migration note. A backward-compatible addition may remain in v4; a change that makes an existing v4 repository unreadable or changes generated behavior incompatibly requires a contract bump. A `pr_manager` loop workflow may set `codex_home` to choose the exact `CODEX_HOME` for its unattended `codex exec` worker; omission inherits the caller environment for compatibility. Bare names resolve only to their conventional home-directory locations, while non-conventional homes require explicit paths. Same-contract-epoch loop JSON preserves the input as `codex_home_configured`; repair-attempt actions and worker receipts report the canonical worker directory as `codex_home_resolved`, while actions that do not attempt a repair omit that field.
