...
The stable generated contract is `.agent/jig-contract.json`. Current renders use `contract_version: 10`, with explicit components, actions, profiles, adapter provenance, literal argv and explicit shell runners, compatibility `jig.*` aliases, typed native configuration, declared bounded string arguments, and target-local matching for non-empty action inputs. Epoch 9 adds explicit exhaustive-input policies and versioned target receipt freshness with original dependency proof and inherited expiry. Omitted input policies remain whole-repository. Epoch 10 adds explicit `source_state = "worktree"` for commands whose results depend on working-file content, allowing receipts to survive staging and commits of unchanged checked files. The default `git` policy retains Git placement and HEAD/branch authority. Native checks retain their Git and comparison dependencies. Epochs 8 and 9 keep their recorded receipt semantics; upgrading requires new evidence without rewriting historical receipts. Contracts v6/v7 retain their released command behavior; v6 keeps component-aggregate matching, and versions 2 through 5 remain readable through the legacy repository projection.
