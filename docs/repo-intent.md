...
`crates/jig-ui` owns the unified interactive terminal application used by both `scripts/jig ui` and `scripts/jig status --tui`. Its typed `DashboardSource` boundary carries local recorder, plan-detail, and status snapshots without exposing `RepoContext`, provider commands, state storage, or runtime policy. The CLI adapter in `crates/jig/src/ui.rs` and `crates/jig/src/ui/source/` owns repository collection and status aggregation. The TUI is read-only: it does not cache status, write receipts, fetch remotes, run actions, or launch agents.
...
The canonical `scripts/jig ui` entrypoint starts on Work and loads status providers only when a status-domain tab is visited or `R` explicitly refreshes both domains. The permanent `scripts/jig status --tui` alias starts on Status and requests providers immediately. Status collection also publishes a fresh local recorder epoch. One-shot `jig ui --json` and `jig ui --plan PLAN_ID --json` use bounded recorder schema 1 without starting the terminal application or any provider. The retired browser transport has no replacement server or HTTP compatibility layer.
