...
- [x] `jig-sh-0zi8.4`: behavioral validation and documentation.

Restart checkpoint: all four tasks and epic `jig-sh-0zi8` are complete on `feat/agent-velocity-improvements`. Structured work finished successfully after required gate verification. Initial checkout was clean at `bed5af6e`. Jig plan: `plan_01M2QP69RTD9RQAPBWC2RYZXPH`, baseline `bed5af6e`. No blocking dependency outside this epic. Build dev runtime before harness commands and set `JIG_DEV_BIN=target/debug/jig`.
