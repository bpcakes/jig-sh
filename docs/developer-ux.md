...
The human status summary uses observed `activity` and cleanup evidence: running, activity uncertain, cleanup required, or stopped. JSON retains the older aggregate `running` rule for compatibility; inspect `activity`, `cleanup_required`, and each session's `retention_reason` when deciding what to clean up. A conflict names each exact session ID and the selected state directory. Inspect from an owning repository with that state directory while its root still exists. The current repository-scoped `dev status` cannot inspect a claim whose saved root has moved or disappeared; retain the exact ID and state directory for repair. `route_present` reports a persisted exact-owned route, not a successful network probe.
