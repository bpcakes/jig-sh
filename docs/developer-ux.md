...
Every foreground launch also registers a tied dev session in the proxy state directory. `scripts/jig dev status` is a read-only view of sessions owned by the current canonical repository, including supervisor/app observations and whether an orphan is `recoverable`; no sessions is a successful stopped state. `scripts/jig dev stop` requests shutdown of every registered session for that repository and is idempotent when nothing is running. It uses the session's authenticated loopback control endpoint so the live supervisor performs its ordinary handle-backed cleanup. If that supervisor is gone while an app spawn is pending or unknown, or while a registered identity remains live or uncertain, the command exits nonzero and retains the session record for inspection; it never converts persisted numeric PIDs into authority to signal a possibly recycled process. If spawn state is known and all exact registered identities are absent, `dev stop` and `dev --replace` recover by retiring the orphan and its exact-owned process routes under the state lock and print a warning describing the recovery. When only pending or legacy-untracked spawn evidence blocks a dead-supervisor orphan, an operator who has independently checked for unrecorded processes can run `scripts/jig dev stop --forget-ambiguous-orphans`. This explicit repair remains fail-closed for every live or uncertain registered identity, signals no persisted PID, and warns that an unrecorded process may still be running.
